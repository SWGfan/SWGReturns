...
int FrsManager::calculatePvpExperienceChange(CreatureObject* attacker, CreatureObject* victim, float contribution, int groupSize, bool isVictim) {
	FrsManagerImplementation* _implementation = static_cast<FrsManagerImplementation*>(_getImplementationForRead());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_CALCULATEPVPEXPERIENCECHANGE__CREATUREOBJECT_CREATUREOBJECT_FLOAT_INT_BOOL_);
		method.addObjectParameter(attacker);
		method.addObjectParameter(victim);
		method.addFloatParameter(contribution);
		method.addSignedIntParameter(groupSize);
		...
		assert((attacker == NULL) || attacker->isLockedByCurrentThread());
		assert((victim == NULL) || victim->isLockedByCurrentThread());
		return _implementation->calculatePvpExperienceChange(attacker, victim, contribution, groupSize, isVictim);
	}
}
...
void FrsManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ISVALIDFRSBATTLE__CREATUREOBJECT_CREATUREOBJECT_:
		...
		break;
	case RPC_CALCULATEPVPEXPERIENCECHANGE__CREATUREOBJECT_CREATUREOBJECT_FLOAT_INT_BOOL_:
		{
			CreatureObject* attacker = static_cast<CreatureObject*>(inv->getObjectParameter());
			CreatureObject* victim = static_cast<CreatureObject*>(inv->getObjectParameter());
			float contribution = inv->getFloatParameter();
			int groupSize = inv->getSignedIntParameter();
			bool isVictim = inv->getBooleanParameter();
			
			int _m_res = calculatePvpExperienceChange(attacker, victim, contribution, groupSize, isVictim);
			resp->insertSignedInt(_m_res);
		}
		break;
...
int FrsManagerAdapter::calculatePvpExperienceChange(CreatureObject* attacker, CreatureObject* victim, float contribution, int groupSize, bool isVictim) {
	return (static_cast<FrsManager*>(stub))->calculatePvpExperienceChange(attacker, victim, contribution, groupSize, isVictim);
}
