...
String CityManager::getNextVoteTimeString(CityRegion* city) {
	CityManagerImplementation* _implementation = static_cast<CityManagerImplementation*>(_getImplementationForRead());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_GETNEXTVOTETIMESTRING__CITYREGION_);
		method.addObjectParameter(city);

		String _return_getNextVoteTimeString;
		method.executeWithAsciiReturn(_return_getNextVoteTimeString);
		return _return_getNextVoteTimeString;
	} else {
		return _implementation->getNextVoteTimeString(city);
	}
}
...
void CityManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_GETNEXTUPDATETIMESTRING__CITYREGION_:
		...
		break;
	case RPC_GETNEXTVOTETIMESTRING__CITYREGION_:
		{
			CityRegion* city = static_cast<CityRegion*>(inv->getObjectParameter());
			
			String _m_res = getNextVoteTimeString(city);
			resp->insertAscii(_m_res);
		}
		break;
...
String CityManagerAdapter::getNextVoteTimeString(CityRegion* city) {
	return (static_cast<CityManager*>(stub))->getNextVoteTimeString(city);
}
