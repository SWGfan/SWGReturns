...
void ManufactureSchematic::initializeSlotsForHeadlessCraft() {
	ManufactureSchematicImplementation* _implementation = static_cast<ManufactureSchematicImplementation*>(_getImplementation());
	if (unlikely(_implementation == NULL)) {
		if (!deployed)
			throw ObjectNotDeployedException(this);

		DistributedMethod method(this, RPC_INITIALIZESLOTSFORHEADLESSCRAFT__);

		method.executeWithVoidReturn();
	} else {
		_implementation->initializeSlotsForHeadlessCraft();
	}
}
...
void ManufactureSchematicAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SETDRAFTSCHEMATIC__DRAFTSCHEMATIC_:
		...
		break;
	case RPC_INITIALIZESLOTSFORHEADLESSCRAFT__:
		{
			
			initializeSlotsForHeadlessCraft();
...
void ManufactureSchematicAdapter::initializeSlotsForHeadlessCraft() {
	(static_cast<ManufactureSchematic*>(stub))->initializeSlotsForHeadlessCraft();
}
