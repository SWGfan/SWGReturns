...
class PobShipStatusSuiCallback : public SuiCallback {
	ManagedWeakReference<PobShipObject*> pobShip;

public:
	PobShipStatusSuiCallback(ZoneServer* serv, PobShipObject* ship) : SuiCallback(serv), pobShip(ship) {
	}
	...
	void run(CreatureObject* creature, SuiBox* sui, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		ManagedReference<PobShipObject*> pobShip = this->pobShip.get();
