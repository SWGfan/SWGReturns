...
int CommandQueue::handleRunningState() {
	...
	try {
		...
		Logger::console.error() << "CommandQueue ERROR -- Creature: " << creature->getDisplayedName() << " ID: " << creature->getObjectID() << " Command Name: " << queueCommand->getQueueCommandName() << " Issue: " << e.getMessage();
		e.printStackTrace();
	}
