/*
		...
		private:
			...
			int performBankTip(CreatureObject* player, CreatureObject* targetPlayer, int amount) const {
				...
				// Get the PlayerObject for the target player
				...
				// Check if the sender and recipient are on the same account
				bool sameAccountTip = (ghost->getAccountID() == targetGhost->getAccountID());
				...
				if (sameAccountTip) {
					...
					promptText = "@base_player:tip_wire_prompt"; // A surcharge of 5% will be added to your requested bank-to-bank transfer amount. Would you like to continue?
				}
		
				// Create the confirmation box
				ManagedReference<SuiMessageBox*> confirmbox = new SuiMessageBox(player, SuiWindowType::BANK_TIP_CONFIRM);
