...
#define COMPANION_TAXI_SPEED_MULTIPLIER 1.05f
#define COMPANION_TAXI_LEASH_DISTANCE_SQ 7225.0f // 85m, squared
#define COMPANION_TAXI_RESUME_DISTANCE_SQ 1225.0f // 35m, squared
...
void CompanionObjectImplementation::interceptThreatToOwner(CreatureObject* attacker) {
	...
	executeObjectControllerAction(STRING_HASHCODE("attack"), attacker->getObjectID(), "");
...
bool CompanionObjectImplementation::startTaxiRide(CreatureObject* owner, float destX, float destY, bool hasDestination, unsigned int vehicleTemplateCRC) {
	...
	if (hasDestination) {
		...
		if (ownerGhost != nullptr) {
			ownerGhost->addSuiBox(goSuiBox);
			owner->sendMessage(goSuiBox->generateMessage());
		}
	}
...
void CompanionObjectImplementation::updateTaxiTick() {
	...
	CompanionGearExchangeManager::scheduleGearCheckTick(_this.getReferenceUnsafeStaticCast());
		...
		if (ownerDistSq >= 0.f) {
			...
			if (taxiThrottled) {
				...
				// 85m leash tripped -- full stop until the owner closes
				// back to 35m. Patrol points stay queued; only the state
				// flips, so resuming continues the same route.
				taxiThrottled = true;
				...
				if (driverCompanion != nullptr) {
					driverCompanion->setCompanionState(CompanionObject::STAY);
					driverCompanion->setOblivious();
				}

				if (rideOwner != nullptr) {
					rideOwner->sendSystemMessage("Your companion pulls over and waits for you to catch up.");
				}
			} else {
				// Driving normally: 5% over the owner's vehicle pace while
				// ahead; 20% extra whenever the owner has overtaken it and
				// pulled more than 35m away ("always keep up").
				float targetRun = taxiBoostedRunSpeed;
				float targetWalk = taxiBoostedWalkSpeed;

				if (!driverAhead && ownerDistSq > COMPANION_TAXI_RESUME_DISTANCE_SQ) {
	...
	CompanionGearExchangeManager::scheduleGearCheckTick(_this.getReferenceUnsafeStaticCast());
		...
		if (rideOwner != nullptr) {
			rideOwner->sendSystemMessage("Your companion sets off -- follow it!");
		}
	}
...
void CompanionObjectImplementation::runPostCombatSweepCheck() {
	...
	for (int i = 0; i < nearbyObjects.size(); ++i) {
		...
		bool lootable = corpseLootableBy(corpse, owner) && corpse->getSlottedObject("inventory") != nullptr && corpse->getSlottedObject("inventory")->getContainerObjectsSize() > 0;
