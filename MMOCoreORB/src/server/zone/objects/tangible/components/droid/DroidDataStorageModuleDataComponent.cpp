...
void DroidDataStorageModuleDataComponent::fillAttributeList(AttributeListMessage* alm, CreatureObject* droid) {
	if (droid != nullptr) {
		...
		// Used Memory
		// This base does not persist the newer per-command size/name metadata.
		// Count each stored command object as one unit so capacity remains visible.
		alm->insertAttribute("droid_program_expended_memory", containerSize);
