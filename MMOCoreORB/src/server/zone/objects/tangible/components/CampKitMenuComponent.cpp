...
int CampKitMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* player, byte selectedID) const {
	...
	if (selectedID == 20) {
		...
		/// Create Structure
		StructureObject* campObject = StructureManager::instance()->placeStructure(player, campTemplate, player->getPositionX(), player->getPositionY(), (int)player->getDirectionAngle());
