...
// Avoid silent “create & hang” cases by sending an error and cleaning up.
...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
    ...
    UnicodeString characterName; callback->getCharacterName(characterName);
    ...
    String raceFile; callback->getRaceFile(raceFile);
    ...
    if (raceData == nullptr) raceData = racialCreationData.get(0);
    ...
    UnicodeString bio; callback->getBiography(bio);
    ...
    if (ghost != nullptr) {
        ...
        if (doTutorial) zoneServer.get()->getPlayerManager()->createTutorialBuilding(playerCreature);
        else            zoneServer.get()->getPlayerManager()->createSkippedTutorialBuilding(playerCreature);
        ...
        Time now; ghost->setBirthDate(now.getTime());
    }
    ...
    // JTL recruitment mail
    if (SendJtlRecruitment* jtlMailTask = new SendJtlRecruitment(playerCreature)) {
        jtlMailTask->schedule(10000);
    }

    // Auction room + welcome SUI + broadcast
    if (ManagedReference<PlayerObject*> ghost2 = playerCreature->getPlayerObject()) {
        ...
        zBroadcast << "\\#00ace6" << playerCreature->getFirstName() << " \\#ffb90f Has Joined Returns!";
        playerCreature->getZoneServer()->getChatManager()->broadcastGalaxy(nullptr, zBroadcast.toString());
    }

    // No movement clamp here (EULA gating handled elsewhere if desired)
    return true;
}

int PlayerCreationManager::getMaximumAttributeLimit(const String& race, int attributeNumber) const {
    String maleRace = race + "_male";
    if (attributeNumber < 0 || attributeNumber > 8) attributeNumber = 0;
    Reference<RacialCreationData*> racialData = racialCreationData.get(maleRace);
    if (racialData != nullptr) return racialData->getAttributeMax(attributeNumber);
    else return racialCreationData.get("human_male")->getAttributeMax(attributeNumber);
}
...
int PlayerCreationManager::getMinimumAttributeLimit(const String& race, int attributeNumber) const {
    ...
    if (attributeNumber < 0 || attributeNumber > 8) attributeNumber = 0;
    Reference<RacialCreationData*> racialData = racialCreationData.get(maleRace);
    if (racialData != nullptr) return racialData->getAttributeMin(attributeNumber);
    else return racialCreationData.get("human_male")->getAttributeMin(attributeNumber);
}

int PlayerCreationManager::getTotalAttributeLimit(const String& race) const {
    String maleRace = race + "_male";
    Reference<RacialCreationData*> racialData = racialCreationData.get(maleRace);
    if (racialData != nullptr) return racialData->getAttributeTotal();
    else return racialCreationData.get("human_male")->getAttributeTotal();
}
...
void PlayerCreationManager::addStartingItems(CreatureObject* creature,
    ...
    for (int i = 0; i < items->size(); ++i) {
        ...
        if (item != nullptr) {
            String error;
            if (creature->canAddObject(item, 4, error) == 0) creature->transferObject(item, 4, false);
            else item->destroyObjectFromDatabase(true);
        }
    }
...
void PlayerCreationManager::addProfessionStartingItems(CreatureObject* creature,
        const String& profession, const String& clientTemplate, bool equipmentOnly) const {
    const ProfessionDefaultsInfo* professionData = professionDefaultsInfo.get(profession);
    if (professionData == nullptr) professionData = professionDefaultsInfo.get(0);
...
void PlayerCreationManager::addHair(CreatureObject* creature,
    ...
    if (hairTemplate.isEmpty()) return;

    HairStyleInfo* hairInfo = hairStyleInfo.get(hairTemplate);
    if (hairInfo == nullptr) hairInfo = hairStyleInfo.get(0);

    HairAssetData* hairAssetData = CustomizationIdManager::instance()->getHairAssetData(hairTemplate);
    if (hairAssetData == nullptr) { error("no hair asset data for " + hairTemplate); return; }

    if (hairAssetData->getServerPlayerTemplate() != creature->getObjectTemplate()->getFullTemplateString()) {
        error("hair " + hairTemplate + " is not compatible with " + creature->getObjectTemplate()->getFullTemplateString());
        return;
    }

    if (!hairAssetData->isAvailableAtCreation()) { error("hair not available at creation: " + hairTemplate); return; }

    ManagedReference<SceneObject*> hair = zoneServer->createObject(hairTemplate.hashCode(), 1);
    if (hair == nullptr) return;

    Locker locker(hair);
    if (!hair->isTangibleObject()) { hair->destroyObjectFromDatabase(true); return; }
    ...
    CustomizationVariables data; data.parseFromClientString(hairCustomization);
...
void PlayerCreationManager::addStartingItemsInto(CreatureObject* creature, SceneObject* container) const {
    ...
    if (playerTemplate == nullptr) { instance()->info("addStartingItemsInto: playerTemplate nullptr"); return; }

    for (int itemNumber = 0; itemNumber < commonStartingItems.size(); itemNumber++) {
        ManagedReference<SceneObject*> item = zoneServer->createObject(commonStartingItems.get(itemNumber).hashCode(), 1);
        if (item != nullptr && container != nullptr && !item->isWeaponObject()) {
            if (!container->transferObject(item, -1, true)) item->destroyObjectFromDatabase(true);
        } else if (item != nullptr) item->destroyObjectFromDatabase(true);
    }

    PlayerObject* player = creature->getPlayerObject();
    if (player == nullptr) { instance()->info("addStartingItemsInto: playerObject nullptr"); return; }
...
void PlayerCreationManager::addStartingWeaponsInto(CreatureObject* creature, SceneObject* container) const {
    ...
    PlayerCreatureTemplate* playerTemplate = dynamic_cast<PlayerCreatureTemplate*>(creature->getObjectTemplate());
    if (playerTemplate == nullptr) { instance()->info("addStartingWeaponsInto: playerTemplate nullptr"); return; }

    PlayerObject* player = creature->getPlayerObject();
    if (player == nullptr) { instance()->info("addStartingWeaponsInto: playerObject nullptr"); return; }
    ...
    for (int itemNumber = 0; itemNumber < commonStartingItems.size(); itemNumber++) {
        ManagedReference<SceneObject*> item = zoneServer->createObject(commonStartingItems.get(itemNumber).hashCode(), 1);
        if (item != nullptr && container != nullptr && item->isWeaponObject()) {
            if (container->transferObject(item, -1, true)) item->sendTo(creature, true);
            else item->destroyObjectFromDatabase(true);
        } else if (item != nullptr) item->destroyObjectFromDatabase(true);
    }
...
void PlayerCreationManager::addRacialMods(CreatureObject* creature,
    ...
    if (racialData == nullptr) racialData = racialCreationData.get(0);
