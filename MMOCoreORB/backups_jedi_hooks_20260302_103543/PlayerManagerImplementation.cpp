...
#include "server/zone/managers/director/ScriptManager.h"
#include <utility>
#include <mutex>
...
void PlayerManagerImplementation::disseminateExperience(TangibleObject* destructedObject, ThreatMap* threatMap,
	...
	for (int i = 0; i < threatMap->size(); ++i) {
		...
		if (attacker->isPet()) {
			...
			// Returns: Jedi unlock hook (kill)
			{
				ManagedReference<CreatureObject*> victim = nullptr;
				if (destructedObject != nullptr && destructedObject->isCreatureObject())
					victim = cast<CreatureObject*>(destructedObject);

				if (attacker != nullptr && attacker->isPlayerCreature() && victim != nullptr) {
					ScriptManager* scriptManager = ScriptManager::instance();
					if (scriptManager != nullptr) {
						LOCK_GUARD(scriptManager->getLock());
						Lua* lua = scriptManager->getLuaInstance();
						if (lua != nullptr) {
							lua_State* L = lua->getLuaState();
							lua_getglobal(L, "jedi_on_kill");
							if (lua_isfunction(L, -1)) {
								lua_pushlightuserdata(L, attacker.get());
								lua_pushlightuserdata(L, victim.get());
								if (lua_pcall(L, 2, 0, 0) != 0) {
									String errMsg = lua_tostring(L, -1);
									lua_pop(L, 1);
									error("Jedi kill hook error: " + errMsg);
								}
							} else { lua_pop(L, 1); }
						}
					}
