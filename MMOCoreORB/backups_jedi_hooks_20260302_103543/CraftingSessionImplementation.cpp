...
void CraftingSessionImplementation::createPrototype(int clientCounter, bool createItem) {
	...
	if (manufactureSchematic->isAssembled()
		...
		// Returns: Jedi unlock hook (craft)
		if (crafter != nullptr && crafter->isPlayerCreature()) {
			...
			if (scriptManager != nullptr) {
				...
				if (lua != nullptr) {
					...
					lua_getglobal(L, "jedi_on_craft");
					if (lua_isfunction(L, -1)) {
						lua_pushlightuserdata(L, crafter.get());
						lua_pushinteger(L, (lua_Integer)xp);
						if (lua_pcall(L, 2, 0, 0) != 0) {
							String errMsg = lua_tostring(L, -1);
							lua_pop(L, 1);
							error("Jedi craft hook error: " + errMsg);
						}
					} else { lua_pop(L, 1); }
				}
			}
